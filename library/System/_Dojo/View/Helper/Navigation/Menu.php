 ...
 * @license    http://framework.zend.com/license/new-bsd     New BSD License
 */
class System_Dojo_View_Helper_Navigation_Menu extends Zend_View_Helper_Navigation_Menu
{
    /**
     * CSS class to use for the div element
     *
     * @var string
     */
    protected $_divClass = '';
    ...
    /**
     * Unique identifier (id) for the div element
     *
     * @var string
     */
    protected $_divId = 'zendDojoNavigation';
    ...
    /**
     * CSS class to use for the parent div element
     *
     * @var string
     */
    ...
    /**
     * Whether parent div elements should be rendered with parent class
     *
     * @var bool
     */
    ...
    /**
     * Adds CSS class from page to div element
     *
     * @var bool
     */
    protected $_addPageClassTochildDiv = false;
    ...
    /**
     * Sets CSS class to use for the first 'div' element when rendering
     *
     * @param  string $divClass                   CSS class to set
    ...
    /**
     * Returns CSS class to use for the first 'div' element when rendering
     *
    ...
    /**
     * Sets unique identifier (id) to use for the first 'div' element when
     * rendering
     *
     * @param  string|null  $divId                Unique identifier (id) to set
     * @return Zend_View_Helper_Navigation_Menu  fluent interface, returns self
     */
    ...
    /**
     * Returns unique identifier (id) to use for the first 'div' element when
     * rendering
     *
    ...
    /**
     * Sets CSS class to use for the parent div elements when rendering
     *
    ...
    /**
     * Enables/disables rendering of parent class to the div element
     *
    ...
    /**
     * Returns flag indicating whether parent class should be rendered to the div
     * element
     *
     ...
     * @return Zend_View_Helper_Navigation_Menu fluent interface, returns self
     */
    public function addPageClassTochildDiv($flag = true)
    {
        $this->_addPageClassTochildDiv = (bool) $flag;
     ...
     * @return bool
     */
    public function getAddPageClassTochildDiv()
    {
        return $this->_addPageClassTochildDiv;
    }
    ...
    public function htmlify(Zend_Navigation_Page $page)
    {
        ...
        if (false === $this->getAddPageClassTochildDiv()) {
        ...
        if ($href = $page->getHref()) {
        	...
        	/* 
        	 * by default this is a span, but it needs to be a dojo div
        	 */
            $element = 'span';
            $attribs['href']      = $href;
            $attribs['target']    = $page->getTarget();
            $attribs['accesskey'] = $page->getAccessKey();
            //$attribs['class'] = 'testClass';
            //$attribs['data-dojo-type'] = 'dijit/PopupMenuItem'; // inner element that wraps the label
        }
    ...
    protected function _normalizeOptions(array $options = array())
    {
        ...
        // UL class
        if (isset($options['divClass']) && $options['divClass'] !== null) {
            $options['divClass'] = (string) $options['divClass'];
        } else {
            $options['divClass'] = $this->getDivClass();
        }

        // UL id
        if (isset($options['divId']) && $options['divId'] !== null) {
            $options['divId'] = (string) $options['divId'];
        } else {
            $options['divId'] = $this->getDivId();
        }
        ...
        if (!isset($options['addPageClassTochildDiv'])) {
            $options['addPageClassTochildDiv'] = $this->getAddPageClassTochildDiv();
        }
     ...
     * @param  string                    $divClass       CSS class for first Div
     * @param  string                    $indent        initial indentation
     * @param  string                    $innerIndent   inner indentation
     * @param  int|null                  $minDepth      minimum depth
     * @param  int|null                  $maxDepth      maximum depth
     * @param  string|null               $divId          unique identifier (id)
     *                                                  for first UL
     * @param  bool                      $addPageClassTochildDiv  adds CSS class from
     *                                                      page to div element
                                          ...
                                          $renderParentClass)
    {
        ...
        $attribs = array(
            'class' => $divClass,
            'id'    => $divId
        	//'data-dojo-type' => 'dijit/PopupMenuBarItem'
        );
        ...
        foreach ($active['page'] as $subPage) {
            ...
            $childDivClass = '';
            if ($subPage->isActive(true) && $addPageClassTochildDiv) {
                $childDivClass = $this->_htmlAttribs(
                    array('class' => $activeClass . ' ' . $subPage->getClass())
                );
            } else if ($subPage->isActive(true)) {
                $childDivClass = $this->_htmlAttribs(array('class' => $activeClass));
            } else if ($addPageClassTochildDiv) {
                $childDivClass = $this->_htmlAttribs(
                    array('class' => $subPage->getClass())
                );
            }
            $html .= $indent . $innerIndent . '<div' . $childDivClass . '>' . $this->getEOL();
            $html .= $indent . str_repeat($innerIndent, 2) . $this->htmlify($subPage)
                                                           . $this->getEOL();
            $html .= $indent . $innerIndent . '</div>' . $this->getEOL();
        }
     ...
     *
     * @param  Zend_Navigation_Container $container     container to render
     * @param  string                    $divClass       CSS class for first UL
     * @param  string                    $indent        initial indentation
     * @param  string                    $innerIndent   inner indentation
     * @param  int|null                  $minDepth      minimum depth
     * @param  int|null                  $maxDepth      maximum depth
                                   ...
                                   $renderParentClass)
    {
        ...
        foreach ($iterator as $page) {
            ...
            if ($depth > $prevDepth) {
                ...
                if (0 == $depth) {
                    $attribs = array(
                        'class' => $divClass,
                        'id'    => $divId,
                    	//'data-dojo-type' => 'dijit/Toolbar' // top level
                    );
                }
                elseif(0 !== $depth && $page->hasChildren()) {
                	$log = Zend_Registry::get('log');
                	$log->debug($depth);
                	$attribs = array(
                			'class' => $divClass,
                			'id'    => $divId,
                			//'data-dojo-type' => 'dijit/DropDownButton' // top level
                	);
                }
                ...
                // close childDiv/div tags until we're at current depth
                for ($i = $prevDepth; $i > $depth; $i--) {
                    //$ind   = $indent . str_repeat($innerIndent, $i * 2);
                    $html .= '</div>' . $this->getEOL();
                    $html .= '</div>' . $this->getEOL();
                }
           ...
           $htmlifyAttribs = array(
           		//'data-dojo-type' =>  'dijit/Menu'
           );
           $default = array();
           $html .= '<div' . $this->_htmlAttribs($htmlifyAttribs) . '>' . $this->getEOL()
            			. $this->htmlify($page)
            			. $this->getEOL();
        ...
        if ($html) {
            // done iterating container; close open div/childDiv tags
...
//                 $myIndent = $indent . str_repeat($innerIndent . $innerIndent, $i - 1);
     ...
     * Renders helper
     *
     * Renders a HTML 'div' for the given $container. If $container is not given,
                               ...
                               array $options = array())
    {
        ...
        if ($options['onlyActiveBranch'] && !$options['renderParents']) {
            $html = $this->_renderDeepestMenu(
                ...
                $options['maxDepth'],
                $options['divId'],
                $options['addPageClassTochildDiv'],
                $options['activeClass'],
                $options['parentClass'],
     ...
     *                                               the helper.
     * @param  string|null               $divClass    [optional] CSS class to
     ...
     *                                               {@link getIndent()}.
     * @param  string|null               $divId       [optional] Unique identifier
                                  ...
                                  $innerIndent = null)
    {
        return $this->renderMenu($container, array(
            ...
            'renderParents'    => false,
            'divId'             => $divId,
            'addPageClassTochildDiv' => $addPageClassTochildDiv,
        ));
    }
     ...
     * will be rendered as an 'div' element by the helper's internal method.
     *
     * @see renderPartial()
     * @see renderMenu()
     *
