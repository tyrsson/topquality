 ...
 *
 */
class System_Service_Authorize_Data_CreditCard
{
	...
	/**
	 * Reformat the data for sending off.
	 *
	 * @return array
	 */
	...
	{
		$data = array();
		$data['x_creditcardtype'] = $this->type;
		$data['x_account']           = $this->accountNumber;
		$data['x_expiry']        = $this->expiration;
		$data['x_cvv2']           = $this->cvv2;
		$data['x_startdate']      = $this->startDate;
	...
	private function _validateAccountNumber($accountNumber)
	{
		...
		if(!$ccValidator->isValid($accountNumber)) {
			...
			require_once 'System/Service/Authorize/Data/Exception.php';
			throw new System_Service_Authorize_Data_Exception($messages[0]);
		}
	}
}
